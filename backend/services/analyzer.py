...
Структура ответа:
{
  ...
  "safe_redaction": "текст пункта, переписанный так, чтобы он стал сбалансированным и не нёс рисков" или null
}
...
- Если is_risky=true (уровень high или medium), предложи переписанный текст этого фрагмента, который устраняет выявленный риск.
- Опирайся ТОЛЬКО на нормы из контекста (если они предоставлены) и на общие принципы ГК РФ. Не выдумывай нормы.
- Сохраняй юридический стиль и структуру исходного фрагмента, меняя только рисковые формулировки.
