...
=== КОРПОРАТИВНЫЕ НОРМЫ (ВЫСШИЙ ПРИОРИТЕТ) ===

Если в RAG-контексте есть блок [КОРПОРАТИВНАЯ НОРМА — ПРИНЯТЫЙ СТАНДАРТ КОМПАНИИ]:
Это означает, что компания явно одобрила данную формулировку.
Если текст сегмента по смыслу СООТВЕТСТВУЕТ корпоративной норме — ОБЯЗАТЕЛЬНО верни:
  is_risky: false, risk_level: none
Корпоративная норма ПЕРЕВЕШИВАЕТ системные рисковые чанки по той же теме.
...
ВАЖНО: если текст сегмента содержит явный признак эталона (компетентный орган,
фактически выполненный объём, пропорционально невыполненным работам, обе стороны
уведомляют) — is_risky: false, даже если RAG-контекст содержит похожий РИСК-паттерн.
Решение принимается по ТЕКСТУ сегмента, РИСК-паттерн служит только для понимания темы.
...
Фрагмент: "Стороны освобождаются от ответственности за неисполнение обязательств вследствие форс-мажора. Наличие форс-мажора подтверждается справкой ТПП."
ЭТАЛОН: "Форс-мажорные обстоятельства подтверждаются справкой компетентного органа"
→ is_risky: false (текст описывает стандартный симметричный форс-мажор с объективным критерием)
...
5. ФОРС-МАЖОР С ОБЪЕКТИВНЫМ КРИТЕРИЕМ (нейтральный):
   Любая формулировка где форс-мажор подтверждается ВНЕШНИМ органом — ТПП, суд, госорган,
   "компетентный орган". Решение о наступлении принимает не одна из сторон.
   Примеры:
   "подтверждённых компетентным органом"
   "Форс-мажор подтверждается справкой компетентного органа (ТПП)."
   → is_risky: false, risk_level: none
   РИСК только если решение за одной стороной: "признанными Заказчиком форс-мажорными"

6. СИММЕТРИЧНОЕ РАСТОРЖЕНИЕ (нейтральный):
   Любая из сторон вправе расторгнуть договор, уведомив другую за N дней.
   Право расторжения равное для обеих сторон.
   Примеры:
   "Каждая из сторон вправе расторгнуть договор, уведомив другую сторону за 14 календарных дней"
   "вправе расторгнуть в одностороннем порядке, письменно уведомив за 30 дней"
   → is_risky: false, risk_level: none
   РИСК только если расторжение асимметрично: только заказчик, или только с согласия одной стороны.

7. ПРЕДМЕТ ДОГОВОРА С ТЕХНИЧЕСКИМ ЗАДАНИЕМ (нейтральный):
   Исполнитель обязуется оказать услуги согласно ТЗ или Приложению, заказчик принять и оплатить.
   Объём определён документом, не усмотрением заказчика в одностороннем порядке.
   Примеры:
   "оказать услуги по разработке веб-сайта в соответствии с Техническим заданием (Приложение № 1)"
   "оказать услуги в объёме, определённом Приложением"
   → is_risky: false, risk_level: none
   РИСК только если: "объём и сроки определяются Заказчиком в одностороннем порядке"
...
9. РЕЗУЛЬТАТ РАБОТ — ПЕРЕЧЕНЬ БЕЗ ОГРАНИЧЕНИЙ (нейтральный):
   Сегмент просто перечисляет что будет передано: сайт, код, документация, модули.
   Нет формулировок "на усмотрение исполнителя", "по требованию", "частично".
   Пример: "Результат работ: функционирующий веб-сайт с описанными в Техническом задании
   модулями, исходный код, документация."
   → is_risky: false, risk_level: none

10. ОТВЕТСТВЕННОСТЬ СТОРОНЫ ЗА СВОИ СОБСТВЕННЫЕ МАТЕРИАЛЫ (нейтральный):
    Заказчик несёт ответственность за достоверность предоставленных им материалов и ТЗ.
    Это стандартное распределение — каждая сторона отвечает за то, что сама предоставляет.
    Пример: "Заказчик несёт ответственность за достоверность предоставленных материалов
    и технического задания."
    → is_risky: false, risk_level: none

11. РАСТОРЖЕНИЕ С ОПЛАТОЙ ФАКТИЧЕСКИ ВЫПОЛНЕННОГО (нейтральный):
    При расторжении по инициативе заказчика — заказчик оплачивает фактически выполненный объём.
    Это защита исполнителя, не риск.
    Пример: "При расторжении по инициативе Заказчика Заказчик оплачивает фактически
    выполненный объём работ на дату расторжения."
    → is_risky: false, risk_level: none
    РИСК только если: "без оплаты выполненного объёма", "без объяснения причин и без оплаты"

12. ВОЗВРАТ АВАНСА ПРОПОРЦИОНАЛЬНО НЕВЫПОЛНЕННОМУ (нейтральный):
    При расторжении по инициативе исполнителя без вины заказчика — возврат аванса
    пропорционально невыполненным работам. Симметричная справедливая формулировка.
    Пример: "При расторжении по инициативе Исполнителя без вины Заказчика Исполнитель
    возвращает аванс в части, соответствующей невыполненным работам."
    → is_risky: false, risk_level: none
...
- Наличие RAG-контекста про интеллектуальную собственность НЕ делает нейтральный перечень
  результатов работ рисковым. Проверяй: есть ли в тексте сегмента ограничение передачи
  или формулировка "на усмотрение"? Если нет — is_risky: false."""
